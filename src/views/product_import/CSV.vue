<template>
  <div>
    ...
    <v-btn color="success" :dark="valid" :disabled="!valid" @click="submit">Import</v-btn>

    <div v-if="successful">
      <p>Your file is successfully uploaded. Your products page will be updated in a short while.</p>
    </div>
    ...
    <div class="pt-10">
      ...
      <ul>
        <li>Only CSV format is acceptable</li>
  ...
  data() {
    return {
      ...
      rules: [
        ...
        v => !!v && v.size < 1024000 || 'File size should be less than 1 MB!',
      ],
    };
  },
  methods: {
    async submit() {
      ...
      if (this.valid) {
        ...
        this.successful = await ImportService.uploadCSV(formData);
        if (this.successful) {
          this.file = null;
        }
        this.loading = false;
